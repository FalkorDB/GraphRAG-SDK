---
title: "Storage"
description: "How GraphStore, VectorStore, and EntityDeduplicator persist data in FalkorDB."
---
...
2. **Group** by `(normalized name, label)` — the name is lower-cased, accent- and punctuation-folded, and stripped of a leading English article, so `The University of Barcelona` and `university of barcelona` fall together. Grouping by label prevents cross-type merging (Person "Paris" and Location "Paris" stay separate). Generational suffixes are kept: `Elias Whitford, Jr.` is not `Elias Whitford`.
3. **Fold acronyms:** a 2–6 letter name that spells the initials of exactly one same-label multi-word name (`AIHS` → `Ashford Island Historical Society`) joins that group. An acronym with two or more plausible expansions (`US` → `United States`, `Universal Studios`) is left alone.
4. **For each group with duplicates:**
   - **Survivor:** The entity with the longest description; the other descriptions are appended, joined with `" | "`, so nothing a duplicate said is lost
...
### Phase 3: LLM-Judged Cross-Document Dedup (Default in `finalize()`)

Ingest resolves per document, so `Airbus` in one file and `Airbus SE` in another never meet until here. `LLMJudgeDeduplicator` (`storage/judge_dedup.py`) runs once over the whole graph, after Phase 1:

1. **Embed** every entity's name (reused from `backfill_entity_embeddings()`, written to `e.embedding` if missing) and its description
...
3. **Group** candidates into dense sets of at most 8 (a chain A–B–C–D where only neighbours are similar cannot form); pack whole sets into prompts of ≤ 3,000 tokens
...
5. **Judge, pass 2** — the same sets, members shuffled and reversed; an answer that changes with the order is a guess
6. **Both passes agree → merge**: survivor = longest description; descriptions joined with `" | "`; every member's label added to the survivor; other names kept in `aliases`; edges remapped to the survivor first, then the loser deleted
7. **Only one pass agrees → link**: a `SAME_AS` edge (`source='llm_judge'`, `agreement=1`) between the two; both nodes and both descriptions stay
8. **Neither → nothing**

Measured on the benchmark corpus: the two-pass vote cut wrong merges by two thirds at 2× judge cost; gpt-4.1 made 9 wrong merges where gpt-4o-mini made 51, so pass a gpt-4.1-class `judge_llm`. Statistics (candidates, sets, calls, agreed/disagreed pairs, merged, linked) are in `EntityDeduplicator.last_judge_stats` and `FinalizeResult.judge_stats`. `finalize(judge=False)` skips this phase.
...
| [`storage/deduplicator.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/storage/deduplicator.py) | EntityDeduplicator — exact + fuzzy dedup, edge remapping; runs the judge phase |
| [`storage/judge_dedup.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/storage/judge_dedup.py) | LLMJudgeDeduplicator — cross-document dedup: nominate, group, judge twice, merge or link |
