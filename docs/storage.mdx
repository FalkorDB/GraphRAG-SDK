...
3. **For each group with duplicates:**
   - **Survivor:** A node a declared table wrote (it carries the row's key, so the table's next re-sync still finds it), then a real node over a placeholder, then the best-connected node (most RELATES and MENTIONED_IN edges), then the longest description
