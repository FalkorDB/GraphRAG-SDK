...
3. **For each group with duplicates:**
   - **Survivor:** A node whose id came from a declared table key, then a real node over a placeholder, then the best-connected node (most RELATES and MENTIONED_IN edges), then the longest description
