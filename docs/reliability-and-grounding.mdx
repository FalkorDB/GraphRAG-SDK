...
| Abstention behavior | Prompt-level only (system prompt rule 6); enforce programmatically with `retrieve()` + a score gate |
...
system prompt whose first rule is *"Answer questions using ONLY the context
provided in the user message"*. The remaining rules require the model to
preserve exact names, dates and negations, and — rule 6 — to state that the
context is insufficient rather than invent details.
