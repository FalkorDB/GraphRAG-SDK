---
title: "Getting Started with GraphRAG SDK in Python"
sidebarTitle: "Getting Started"
description: "Install GraphRAG SDK, connect to FalkorDB, and build your first knowledge graph from documents in a step-by-step Python tutorial."
---

This tutorial walks through installing GraphRAG SDK, starting FalkorDB, and building your first knowledge graph from documents so you can query it with natural language.
...
`ConnectionConfig` accepts additional parameters such as `port`, `username`, `password`, and `query_timeout_ms`. See [Configuration](/configuration) for the full list.

> **Alternative providers:** The SDK also exports `OpenRouterLLM` and `OpenRouterEmbedder` for use with OpenRouter. See [Configuration](/configuration) for details.
...
- [Configuration](/configuration) -- Tuning connection settings, chunking parameters, and retrieval options.
- [Strategies](/strategies) -- Custom extraction and resolution strategies.
- [Benchmark](/benchmark) -- Reproducing benchmark results on the GraphRAG-Bench Novel corpus (20 novels, 2,010 questions).
