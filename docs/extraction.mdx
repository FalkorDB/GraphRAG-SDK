---
title: "GraphRAG SDK Entity & Relation Extraction"
sidebarTitle: "Extraction"
description: "How GraphRAG SDK's hybrid NER-plus-LLM extraction turns raw text chunks into typed entities and the relationships between them."
---
...
extractor = GLiNERExtractor(
    threshold=0.75,                          # at or above: typed entity (default: None = per-model value)
    candidate_threshold=0.5625,              # band below threshold kept as "Unknown" (default: 25 % below threshold)
    model_name="urchade/gliner_medium-v2.1", # HuggingFace model (default)
    window_tokens=None,                      # words per inference window (default: derived from the model's max_len)
    window_overlap=48,                       # words shared by consecutive windows
)
```
...
3. Text longer than the model's input limit (`config.max_len`, 384 words for the default model) is processed as overlapping windows and merged back into non-overlapping spans, so the tail of a long chunk is not truncated. Short text takes a single-pass fast path.
4. Returns predictions with character-level spans — more precise than LLM spans
5. Entities in the band just below the confidence threshold (default: 25 % below it, e.g. 0.5625–0.75) are labeled `"Unknown"` and handed to the step-2 LLM, which re-types or drops them against the text; anything below the band is discarded. Pass `candidate_threshold=None` to disable the band.

Thresholds are model-specific: `threshold=None` (the default) selects the measured value for `model_name` (`0.75` for the `urchade/gliner_*` models, `0.5` for the `knowledgator/gliner-bi-*` bi-encoders); unknown models fall back to `0.5` with a warning. Invalid configurations raise `ValueError`: `threshold` outside `[0, 1]`, `candidate_threshold` outside `[0, threshold]`, `window_tokens` not a positive integer, or `window_overlap` not a non-negative integer — all at construction. `window_overlap` must also be smaller than the window; that is checked at construction when `window_tokens` is given explicitly, otherwise on the first inference call, once the window has been derived from the model.
...
Relations resolve the same way, with one difference in strictness:

1. **`Ontology.relations`** (highest priority). The prompt lists them under *Allowed Relationships* and says the type MUST be one of them, and `IngestionPipeline` prunes any edge whose `rel_type` is not declared.
2. **`relation_types` parameter on GraphExtraction.** Guidance, not a filter: the prompt lists them under *Preferred Relationships* and asks the LLM to prefer one, falling back to a descriptive `UPPER_SNAKE_CASE` label when none fits. Off-list edges are kept. Blank labels are rejected.
3. **`DEFAULT_RELATION_TYPES`** (lowest priority). A 31-label domain-neutral vocabulary (`LOCATED_IN`, `PART_OF`, `CONTAINS`, `MEMBER_OF`, `EMPLOYED_AT`, `FOUNDED`, `CREATED`, `AUTHORED`, `BORN_IN`, `MARRIED_TO`, `USES`, ...) used when neither of the above is given. Without it the model invented a fresh label for almost every edge (447 distinct labels against 30 in the benchmark answer key); with it the vocabulary shrinks ~4x and exact triple F1 doubles.

```python
extractor = GraphExtraction(llm=llm, relation_types=["INHIBITS", "ACTIVATES", "BINDS"])

# Open vocabulary -- the model names every edge itself:
extractor = GraphExtraction(llm=llm, relation_types=[])
```
