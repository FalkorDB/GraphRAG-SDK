...
**Phase 1 (always runs): Exact name match.** Groups entities by normalized name (case, accents, punctuation and a leading English article folded; `A.I.` = `AI`) and label to prevent cross-type merging. An acronym joins its long form when exactly one same-label entity spells its initials (`AIHS` / `Ashford Island Historical Society`). The long form survives; among like names, the entity with the longest description does. The duplicate's description is appended to the survivor's with `" | "`, its name is recorded in the survivor's `aliases`, its `source_chunk_ids` are unioned in, and its RELATES and MENTIONED_IN edges are remapped before it is deleted.
