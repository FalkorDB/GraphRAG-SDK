---
title: "GraphRAG SDK Configuration Reference"
sidebarTitle: "Configuration"
description: "Comprehensive GraphRAG SDK configuration reference -- every configurable component, its parameters, defaults, and usage examples."
---
...
merged_count = await rag.deduplicate_entities(
    ...
    similarity_threshold=0.95,    # cosine threshold for fuzzy matching
...
| `similarity_threshold` | `float` | `0.95`  | Cosine similarity threshold for fuzzy matching.                     |
| `batch_size`           | `int`   | `500`   | Entities per query batch.                                           |

**Phase 1 (always runs): Exact name match.** Groups entities by canonical name (case, accents, punctuation and a leading English article folded; `A.I.` = `AI`; `Surname, Given` un-inverted; common abbreviations expanded; one trailing legal form such as `Ltd` or `Inc` dropped) and label to prevent cross-type merging. An acronym joins its long form when exactly one same-label entity spells its initials (`AIHS` / `Ashford Island Historical Society`). The survivor is the node whose id came from a declared table key, then a real node over a placeholder, then the long form over an acronym, then the best-connected entity (most RELATES and MENTIONED_IN edges), then the longest description. The duplicate's description is appended to the survivor's with `" | "`, its name is recorded in the survivor's `aliases`, its `source_chunk_ids` are unioned in, any property the survivor lacks is copied over, and its RELATES and MENTIONED_IN edges are remapped before it is deleted.
