...
**Phase 1 (always runs): Exact name match.** Groups entities by normalized name (lowercase, stripped) and label to prevent cross-type merging. Keeps the best-connected entity (most RELATES and MENTIONED_IN edges) as the survivor, falling back to the longest description on a tie; a node whose id came from a declared table key always outranks one extracted from prose. Remaps all RELATES and MENTIONED_IN edges from duplicates to the survivor, then deletes the duplicate nodes.
