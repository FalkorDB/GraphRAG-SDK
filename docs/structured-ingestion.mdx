...
Three, shortest first. All run against a local FalkorDB.
...
| `examples/14_research_group_knowledge_base/` | A realistic corpus that fits the design about 80%: twelve CSVs written by different people (inconsistent keys, a bare junction table, an undeclared file), six arXiv PDFs of which two connect only through prose and one must stay an island, two notes and a JSON dump. Separate scripts build the graph, verify it against the CSVs with Cypher, ask 18 aggregate/hybrid/lookup questions and load six deliberately faulty files. Its README records what the SDK does and does not do with such data. |
