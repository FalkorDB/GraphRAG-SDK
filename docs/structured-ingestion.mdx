...
the labels in use with their entity counts, the relationship types, the tables
already mapped and their key columns. It answers the one thing measurement cannot
settle, which is what a row is *about*: the label, the name column, the key, the
type of every other column, and which columns are links to other entities.
