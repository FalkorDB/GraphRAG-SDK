...
Two headers that reduce to one stored name (`HQ Country` and `HQ-Country`) are
both kept: the second is suffixed in header order, `col_hq_country_2`, on the
entity and on the record chunk alike. The key's slot is spoken for first — a
column literally called `col_id` beside `key="id"` is stored as `col_id_2`, and a
declaration that maps a property onto the key's slot is refused, because that
property would otherwise be written over the row's identity.
...
- A label a link points at has **one key column**. Two tables may both describe `Person` — `hr.csv` keyed by `employee_id`, `crm.csv` by `contact_id` — and a person in both exports is one node holding both keys. But a `Link(to="Person", by="lead_id")` could not tell which of the two id spaces `lead_id` means, so whichever declaration completes that shape — the second key or the link — is refused, naming all three. Key both tables by the same column, or give the rows the link means a label of their own.
...
`drop_entity()` removes a label nothing else does — while a table maps rows to it or
links to it, `drop_entity()` refuses and names the table, since the mapping would
re-register the label on the next load. `rename_entity()` follows through: the
stored mapping and every link to the label are renamed with it, and the next
export re-syncs its rows under the new label.
...
The hash is written **last**, and only when every write before it held. A write that raises leaves the Document without one; an edge write the store logged and came up short on is reported as `incomplete_writes` on the result and withholds it too. Either way the next `ingest()` of the same file is not a no-op — it re-runs in full and repairs the table — so a load that failed part-way is never mistaken for a finished one.
</Note>
...
preferred, and there is deliberately no precedence rule. `finalize()` reports
every property two tables both supply — with how many entities hold a value from
more than one of them, and on how many of those the values differ — because the
overlap is also the thing that makes a question ambiguous: asked for "the grade",
a query has to pick a source, and it will pick one silently:
...
A value of another type counts as different: `3` and `"3"` are two answers.
...
What `finalize()` still does is handle **spelling variants**: `NORTHWIND ENERGY` from a NER model and `Northwind Energy` from a CSV are different strings and so different ids. It groups every entity by `(canonical name, label)` and merges each group. The canonical form is reduced independently on both sides — case and accents
folded, punctuation split, abbreviation dots removed (`A.I.` = `AI`), a leading
English article dropped, `Surname, Given` un-inverted, abbreviations expanded,
one trailing legal form dropped — so the join is an equality rather than a
pairwise comparison and the order two sources arrive in does not change what
merges. An acronym also joins the one same-label entity whose initials it spells. Word
