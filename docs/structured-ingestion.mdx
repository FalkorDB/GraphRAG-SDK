...
| Label       | One per                | Notes                                                                 |
| ----------- | ---------------------- | --------------------------------------------------------------------- |
| `Document`  | source file            | Addressed by `document_id`, which defaults to `os.path.normpath(source)`. |
...
Two sources describing one person — `employees.csv` and a board-review PDF — produce this shape. Every node and edge below is real: it is the graph `examples/13_documents_and_tables.ipynb` builds, read back from FalkorDB.
...
| `links`     | Columns that point at other entities. Each becomes a reference node plus an edge, written **ON CREATE only**, so a pointer can never overwrite real data. That is what makes the order of two files irrelevant. Two links to the same label are kept apart automatically. |
...
Nothing is refused. A table nobody mapped still belongs in the graph, so it gets
the *natural transformation*:

```python
result = await rag.ingest("readings.csv")   # nothing declared for it
```

- Every column becomes a typed property, with types measured over the **whole
  file** rather than a sample. A column holding integers for five hundred rows and
  `"N/A"` on row five hundred and one would be declared `INTEGER` from a sample
  and then fail the load it was meant to describe.
- The label comes from the file name, in the file's own casing — `readings`, not
  `Readings` — so it is obvious in the browser which labels nobody declared.
- The key is the leftmost column that is unique and complete.
- There is **no name column, so it joins nothing.** Which column identifies the
  thing a row is *about* is a guess, and a wrong guess silently attaches rows to
  the wrong entities. The rows land queryable and unreachable from any document
  instead.

A WARNING says so at load, naming the label and the key it chose:

```text
No table mapping is declared for readings.csv, so it was read as-is: label
'readings', keyed on 'reading_id', and with no name column it does not join
anything the documents mention. […] Declare a TableMapping in the ontology to
connect it.
```

And `finalize()` reports it, so it is visible after the fact rather than only in a
log:

```python
summary = await rag.finalize()
summary.tables_without_a_mapping   # ["readings.csv"]
```

The fix is to declare a mapping. A file with no unique-and-complete column at all
is refused, because a row with no stable identity could never be updated or
deleted later:
...
A document remembers how it was written, and nothing may change its mind: re-reading a CSV as prose replaced its record chunks with one text chunk and took every entity with them — measured as two organizations before the call and none after, with nothing raised. A table appearing in `apply_changes(modified=[...])` therefore comes back as a per-entry error rather than being acted on. Re-ingest it with `ingest()` instead, which finds its declaration.
...
entity survives — correctly, it is still mentioned — but the first source's values
stay on it, belonging to nobody and reading as current. `finalize()` reports those
as `stale_signed_properties` rather than removing them: the graph cannot tell "that
source dropped the row" from "that source has not been reloaded yet", and guessing
would delete live data. Non-empty means re-load the named source, or accept that
...
Reported rather than merged on purpose. Merging in the graph does not hold — the next time that document is read, extraction recreates the node the merge deleted and you are back to two, measured — so the fix is the spelling in the source, which fixes it for good. Choosing join-able names is the design decision that decides whether this feature works on your data. To check what did join, count the entities reachable from more than one source:
...
`finalize()` folds each group with the ordinary resolver and carries the table's typed columns onto the survivor:
...
Two, shortest first. Both run against a local FalkorDB.

| | |
| --- | --- |
| `examples/13_documents_and_tables.ipynb` | **Start here.** Two mappings in one ontology, then a document, a table, a document and a table, in that order, and finally a corrected version of the first table. It stops after each stage so the graph can be inspected: the employee export arrives before the organizations it points at, so you can watch the reference nodes get filled in rather than duplicated. |
| `examples/11_structured_ingestion.py` | The whole path: two CSVs, a document, resolution, the declared ontology, all three classes of question, and a re-ingest that corrects a row, adds one and drops one. |
