...
| `resolver` | `ResolutionStrategy \| None` | `None` | Custom resolver (ExactMatchResolution if None; cross-document dedup is `finalize()`'s judge phase) |
...
- **Phase 1** (always): Exact name match by `(normalized name, label)` -- keeps longest description, joins descriptions with `" | "`, records absorbed names in `aliases`, remaps RELATES and MENTIONED_IN edges, deletes duplicates.
- **Phase 2** (optional, `fuzzy=True`): Embedding-based -- embeds entity names, finds near-duplicates by cosine similarity.
- **Phase 3** (default, `judge=True`): LLM-judged cross-document dedup -- name/description embeddings and name-in-description nominate candidate pairs, dense sets of ≤ 8, the judge partitions each set, a second shuffled pass must agree; agreed pairs merge, disagreements become `SAME_AS` edges. `judge_llm` defaults to the GraphRAG LLM (gpt-4.1-class recommended); `judge=False` keeps the call LLM-free. Stats in `_deduplicator.last_judge_stats`.
...
```python
async def finalize(*, judge: bool = True, judge_llm: LLMInterface | None = None, judge_vote: bool = True) -> FinalizeResult
```
...
2. `deduplicate_entities(judge=..., judge_llm=..., judge_vote=...)` -- exact-name dedup, then the LLM-judged cross-document phase
3. `embed_relationships()` -- fact text embeddings on RELATES edges
4. `ensure_indices()` -- all indexes

**Returns:** `FinalizeResult` with `null_stubs_removed`, `entities_deduplicated`, `entities_linked`, `judge_llm_calls`, `judge_stats`, `entities_embedded`, `relationships_embedded`, `indexes`.
...
def finalize_sync(*, judge: bool = True, judge_llm: LLMInterface | None = None, judge_vote: bool = True) -> FinalizeResult
...
**Built-in:**
- `ExactMatchResolution(resolve_property="name")` — `ingest()` default
- `LLMVerifiedResolution(llm=None, embedder=None, hard_threshold=0.95, soft_threshold=0.65, batch_verification=True, verification_max_pairs_per_call=5, ...)` — opt-in at `ingest()`
