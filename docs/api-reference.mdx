...
- **Phase 1** (always): Canonical name match — case, punctuation, `Surname, Given` inversion, common abbreviations and one trailing legal form (`Ltd`, `Inc`) are normalised; word order is not. Keeps the survivor ranked by declared key, real-over-placeholder, degree, then description length; remaps RELATES and MENTIONED_IN edges; carries the duplicate's properties over; deletes duplicates.
