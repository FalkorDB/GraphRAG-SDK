...
- **Phase 1** (always): Exact name match -- folds accents, punctuation and acronyms, keeps the long form (else the longest description), merges the duplicate's description, `aliases` and `source_chunk_ids` into the survivor, remaps RELATES and MENTIONED_IN edges, deletes duplicates.
