...
class TableMapping:
    ...
    properties: dict[str, Column | str] = {}     # property_name -> Column; a str means STRING
    links: list[Link] = []                       # columns that point at other entities
...
class CsvRecordLoader(RecordLoaderStrategy):
    def __init__(self, *, delimiter: str | None = None, encoding: str = "utf-8", document_id: str | None = None): ...
```

`CsvRecordLoader` is the default; `delimiter=None` sniffs comma, semicolon, tab or pipe from the first kilobyte. Implement `RecordLoaderStrategy` for another format and pass it as `ingest(..., record_loader=...)`.
