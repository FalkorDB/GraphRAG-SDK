---
title: "GraphRAG SDK API Reference"
sidebarTitle: "API Reference"
description: "Complete API reference for every public class and method exported by graphrag_sdk, from the GraphRAG entry point to strategy ABCs."
---
...
| `resolver` | `ResolutionStrategy \| None` | `None` | Custom resolver (ExactMatchResolution if None; cross-document dedup is `finalize()`'s resolver and judge phases) |
...
Post-ingestion entity deduplication across all documents.
...
- **Phase 4** (default, `judge=True`): LLM-judged cross-document dedup -- name/description embeddings and name-in-description nominate candidate pairs, dense sets of ≤ 8, the judge partitions each set, a second shuffled pass must agree; agreed pairs merge by the same rules as every other phase (the survivor additionally gains every member's label), disagreements become `SAME_AS` edges, and pairs a resolver remembered as `DISTINCT_FROM` are never asked. `judge_llm` defaults to the GraphRAG LLM (gpt-4.1-class recommended); `judge=False` skips the phase. Stats in `_deduplicator.last_judge_stats`.
...
1. `backfill_entity_embeddings()` -- name embeddings on every entity (first, so the judge reuses them)
2. `deduplicate_entities(resolver=..., judge=..., judge_llm=..., judge_vote=...)` -- canonical-name dedup, then the resolver across sources, then the LLM-judged cross-document phase
...
| `resolve` | `bool` | `True` | `False` skips the resolver entirely. |
| `judge` | `bool` | `True` | Run the LLM-judged cross-document phase after the resolver: similarity nominates candidate sets, the judge model partitions them twice, only agreed pairs merge, disagreements become `SAME_AS` edges. `False` skips it; `resolve=False, judge=False` calls no model at all. |
| `judge_llm` | `LLMInterface \| None` | `None` | Judge model; defaults to the instance's `llm`. A gpt-4.1-class model is recommended (9 wrong merges vs 51 for gpt-4o-mini on the benchmark corpus). |
| `judge_vote` | `bool` | `True` | Require the second shuffled pass to agree before merging. `False` halves judge LLM cost at ~3x the wrong-merge rate. |
...
def finalize_sync(*, resolver=None, resolve=True, judge=True, judge_llm=None, judge_vote=True) -> FinalizeResult
...
**Built-in:**
- `ExactMatchResolution(resolve_property="id")` — `ingest()` default
- `LLMVerifiedResolution(llm=None, embedder=None, *, hard_threshold=0.95, soft_threshold=0.80, max_llm_pairs=500, max_llm_concurrency=None, force_summary_threshold=3, max_summary_tokens=500, ann_top_k=50)` — opt-in at `ingest()`; `finalize()`'s default cross-source resolver
