...
### Built-in: LLMVerifiedResolution

Three-tier resolution that catches near-duplicates exact match misses (e.g. `Acme Corp` vs `Acme Corporation`):

1. `(normalized name, label)` exact-match merge -- the same first pass `ExactMatchResolution` performs.
2. Embed surviving node names and compare pairs within each label group:
   - similarity >= `hard_threshold`: merged immediately, no LLM call
   - `soft_threshold` <= similarity < `hard_threshold`: sent to the LLM for a YES/NO verdict
   - similarity < `soft_threshold`: skipped
3. Merge the confirmed clusters, then remap and deduplicate relationships.

Descriptions are merged the same way in both resolvers: concatenated below `force_summary_threshold`, LLM-summarized at or above it.
...
resolver = LLMVerifiedResolution(
    ...
    soft_threshold=0.80,           # Below this, skip the pair entirely (default: 0.80)
...
Both arguments are optional: with no `embedder` it degrades to exact match, and with no `llm` the ambiguous zone is skipped so only the `hard_threshold` tier merges.

**When to use:** Multi-document ingestion where the same entity is named inconsistently across documents. Costs embedding calls on every ingest plus an LLM call when any pair lands in the ambiguous zone.
