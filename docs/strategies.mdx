...
description: "Every swappable strategy ABC and its built-in implementations."
---
...
| 4 | Resolution | `ResolutionStrategy` | `ExactMatchResolution`, `LLMVerifiedResolution` |
...
# GLiNER with custom threshold. Spans scoring in the band just below it
# (here 0.45–0.6) are kept as "Unknown" for the step-2 LLM to re-type or
# drop; pass candidate_threshold=None to discard them instead.
extractor = GraphExtraction(
    llm=llm,
...
|-------|-------------|------------|
| `GLiNERExtractor` | Local GLiNER model (default, no API calls) | `threshold=None` (per-model default, 0.75 for `urchade/gliner_medium-v2.1`), `model_name=None` (`urchade/gliner_medium-v2.1`), `candidate_threshold` (default 25 % below `threshold`), `window_tokens=None`, `window_overlap=48` |
...
Both extractors use `threshold` as the confidence cut-off for a typed entity, but they treat spans below it differently. `LLMExtractor` keeps every span it is given and labels those with confidence below `threshold` as `"Unknown"`. `GLiNERExtractor` only keeps spans down to `candidate_threshold` (by default 25 % under `threshold`) — those in that band are labeled `"Unknown"` and handed to step 2, and anything lower is discarded inside the model and never reaches the pipeline; see [Extraction](/extraction) for details.
...
resolver = ExactMatchResolution(
    resolve_property="name",  # property to match on (default: "name")
)
```

**When to use:** Default. Extraction is consistent, or ingest cost matters more than duplicate recall. Ingest is per document, so cross-document duplicates with different surface forms (`Airbus` / `Airbus SE`) are left for `finalize()`, where the resolver pass and the LLM-judged phase see the whole graph (see [Storage](/storage)).
...
1. `(normalized name, label)` exact-match merge -- the same first pass `ExactMatchResolution` performs.
2. Embed the surviving nodes as `"name: description"` and compare pairs in one unified same/cross-label candidate stage:
   - `similarity >= hard_threshold`: merged immediately, no LLM call
   - `soft_threshold <= similarity < hard_threshold`: sent to the LLM for a YES/NO verdict, several pairs per call
   - `similarity < soft_threshold`: skipped
   A label-family gate keeps pairs that cannot be one thing (Person vs Place) away from the LLM, and a cross-label merge needs a second, A/B-swapped vote to agree.
3. Merge the confirmed clusters, then remap and deduplicate relationships.

Merged descriptions are kept as a list and joined with `" | "` (concatenated below `force_summary_threshold`, LLM-summarized at or above it); labels absorbed from cross-label merges are kept in `merged_labels` and promoted to real Cypher labels on write.

```python
from graphrag_sdk import LLMVerifiedResolution

resolver = LLMVerifiedResolution(
    llm=llm,                       # LLMInterface for YES/NO verification and summaries (optional)
    embedder=embedder,             # Embedder for pairwise cosine similarity (optional)
    hard_threshold=0.95,           # Merge without asking the LLM (default: 0.95)
    soft_threshold=0.65,           # Below this, skip the pair entirely (default: 0.65)
    max_llm_pairs=500,             # Cap on ambiguous pairs per call, highest similarity first (default: 500)
    batch_verification=True,       # Several pairs per LLM call (default: True)
    verification_max_pairs_per_call=5,
    ...
    ann_top_k=50,                  # Nearest neighbours retrieved per node (default: 50)
)
```

Both providers are optional but must be passed explicitly when you construct the strategy yourself; `GraphRAG.ingest()` wires its own LLM and embedder only into the default instance. With no `embedder` it degrades to exact match, and with no `llm` the ambiguous zone is skipped so only the `hard_threshold` tier merges.
...
Production-grade retrieval with RELATES edge vector search, 2-path entity discovery, 4-path chunk retrieval, and cosine reranking. This is the **default**, and the strategy used for the [GraphRAG-Bench results](/benchmark).
