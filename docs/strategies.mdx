...
All extractors share the same `threshold` behavior: entities with confidence below the threshold are labeled `"Unknown"`. `GLiNERExtractor` additionally discards spans below `candidate_threshold` (by default 25 % under `threshold`) inside the model, so only the band just below the threshold reaches step 2 as `"Unknown"`; see [Extraction](/extraction) for details.
