...
### Built-in: LLMVerifiedResolution (default)

Three tiers: exact match by `(normalized name, label)`, then name-embedding similarity, then an LLM YES/NO verdict for the ambiguous zone. A final cross-label pass lets the LLM merge the same thing typed differently (`Fresnel lens` as Product and Technology). Same-name entities of different types are never merged without an LLM verdict.
