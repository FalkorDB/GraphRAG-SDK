...
| 4 | Resolution | `ResolutionStrategy` | `LLMVerifiedResolution`, `ExactMatchResolution` |
...
### Built-in: LLMVerifiedResolution (opt-in at ingest)

Three tiers: exact match by normalized name, then `"name: description"` embedding similarity in one unified same/cross-label candidate stage, then an LLM YES/NO verdict for the ambiguous zone — with a label-family gate (Person vs Place pairs never reach the LLM) and a second, A/B-swapped vote before any cross-label merge. Runs per document when passed to `ingest(resolver=...)`. Cross-document dedup is done once, by the LLM-judged phase in `finalize()` (see [Storage](/storage)).
...
resolver = LLMVerifiedResolution(
    llm=llm,                            # LLMInterface for verification — required for tiers 2-3
    embedder=embedder,                  # Embedder for name similarity — required for tiers 2-3
    hard_threshold=0.95,                # merge without asking at or above this cosine
    soft_threshold=0.65,                # skip pairs below this cosine
    batch_verification=True,            # 5 pairs per LLM call
    verification_max_pairs_per_call=5,
)
```

Both providers must be passed explicitly when you construct the strategy yourself; `GraphRAG.ingest()` wires its own LLM and embedder only into the default instance. Without an embedder the strategy silently runs the exact-match tier alone.

Merged descriptions are joined with `" | "`; labels absorbed from cross-label merges are kept in `merged_labels` and promoted to real Cypher labels on write.

**When to use:** Default. Recall on real duplicates is roughly 2.5x that of exact match on the benchmark corpus, at the cost of embedding and LLM calls during ingest.

### Built-in: ExactMatchResolution (ingest default)

Deduplicates by `(normalized name, label)` only. Fast, no embeddings, no LLM calls.
...
**When to use:** Extraction is consistent, or ingest cost matters more than duplicate recall. Cross-document duplicates with different surface forms (`Airbus` / `Airbus SE`) are left for `finalize()`.
