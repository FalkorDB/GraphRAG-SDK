...
description: "Bootstrap an ontology from a corpus and propose schema additions as new documents arrive."
---

How to bootstrap an ontology from a corpus and how to propose schema additions as new documents arrive — the *discovery* side of the ontology lifecycle. The complement to [Ontology Evolution](/ontology-evolution), which covers safely *changing* an ontology that already exists.
...
1. **The built-in default.** If you construct `GraphRAG` without an `ontology=` argument and no ontology has been previously persisted to the graph, the SDK seeds the ontology graph with `DEFAULT_ENTITY_TYPES` (`Person`, `Organization`, `Technology`, `Product`, `Location`, `Date`, `Event`, `Concept`, `Law`, `Dataset`, `Method`) so extraction has something to anchor on. Relations have a counterpart, `DEFAULT_RELATION_TYPES` (`LOCATED_IN`, `PART_OF`, `CONTAINS`, `MEMBER_OF`, `EMPLOYED_AT`, `FOUNDED`, `AUTHORED`, `BORN_IN`, `USES`, ... 31 labels), but it behaves differently: it is offered to the extraction prompt as a *preferred* vocabulary and is **not** seeded into the ontology graph, so it never activates relation pruning and an off-list label the model emits is kept.
...
4. **Discovered and then evolved.** The realistic workflow: draft once with discovery, ingest with that draft, then as new documents arrive use `suggest_schema_extensions` to propose additions you review and apply via the [evolution API](/ontology-evolution).
...
See [Ontology Evolution](/ontology-evolution) for the invariants `add_attribute` enforces. Discovery never commits anything itself — the existing evolution machinery stays the single commit surface.
...
- [Ontology Evolution](/ontology-evolution) — what to do with the schema once you've discovered it
- [Graph Schema](/graph-schema) — the structural reference for `Ontology`, `Entity`, `Relation`, `Attribute`
