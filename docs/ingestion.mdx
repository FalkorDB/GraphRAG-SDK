...
```
              ...
              (remove bad nodes/dangling edges)
                    |
              5. Prune Against Ontology
              (keep only ontology-conforming data)
                    |
...
**What it does:** Filters extracted data to only keep entities and relationships that match your ontology definition.

**How it works:**
- If your ontology defines entity types (e.g., Person, Organization, Location), only entities with those labels pass through
- If your ontology defines relationship types, only those relationship types pass through
...
**Open ontology mode:** If you define no entity or relationship types (empty `Ontology()`), this step is skipped entirely — everything passes through.
