---
title: "Document Ingestion Pipeline in GraphRAG SDK"
sidebarTitle: "Ingestion"
description: "How GraphRAG SDK turns raw documents into a knowledge graph through a 9-step ingestion pipeline, from chunking to entity resolution."
---
...
[Structured Ingestion](/structured-ingestion): you declare what the columns mean
...
For a detailed explanation of the extraction process, see [Extraction](/extraction).
...
**Default at ingest: ExactMatchResolution**
- Groups by `(normalized name, label)` only — same name but different labels stay separate (e.g., Person "Paris" vs Location "Paris"); the judge decides those at `finalize()`
- Survivor keeps every member's description joined with `" | "` (no LLM summary); all relationship endpoints are re-pointed to the survivor before the write, so no edge is lost
- No embeddings, no LLM calls; zero added cost
- Ingest is per document, so a resolver here can only compare one file's entities with each other — the cross-document duplicates that matter are handled once, at `finalize()`

**Alternative: LLMVerifiedResolution**
- Runs the same exact-match pass first, then compares the surviving names by embedding similarity within each label
- Pairs at or above `hard_threshold` (0.95) merge outright; pairs down to `soft_threshold` (0.65) go to the LLM for a YES/NO verdict
- Useful when the same entity is named inconsistently across documents (e.g., "Acme Corp" vs "Acme Corporation")

**At `finalize()`: the LLM-judged cross-document phase**
...
- The judge LLM partitions each set into same-referent groups; with `judge_vote=True` (default) a second pass over shuffled sets must agree, with `judge_vote=False` the single pass decides (a pair that reached across two sets is still linked rather than merged, with `agreement=1`)
- Both passes agree → **merge**, by the same rules as every finalize merge: a table's row survives a mention, every member's description is kept in `descriptions` (joined with `" | "` in `description`), other names kept in `aliases`, edges moved first, then the loser deleted; the survivor also gains every member's label
- Only one pass agrees → **link**: a `SAME_AS` edge (`source='llm_judge'`), both nodes stay
- Pairs a resolver remembered as `DISTINCT_FROM`, two keyed rows of a table, and a mention two such rows could own are never asked, grouped or linked
- `finalize(judge=False)` skips it (`finalize(resolve=False, judge=False)` runs no model at all); `deduplicate_entities()` on its own runs it only with `judge=True`; `judge_llm=` picks the model (gpt-4.1-class recommended); pass `resolver=LLMVerifiedResolution(...)` to `ingest()` to also resolve per document

For details on resolution strategies, see [Strategies](/strategies).
...
For details on the storage layer, see [Storage](/storage).
...
## Ingesting Many Documents Efficiently

Pass a list of sources to one `ingest()` call to enable document-level
parallelism. Calling `ingest()` once per file waits for each complete pipeline
run before starting the next file.

```python
sources = ["docs/one.pdf", "docs/two.pdf", "docs/three.pdf"]

# Runs up to three document pipelines at once (the default).
results = await rag.ingest(sources, max_concurrency=3)

# Inspect per-source failures: a batch returns an IngestionResult or Exception
# for every source, in input order.
failures = [result for result in results if isinstance(result, Exception)]
```

Increase `max_concurrency` gradually when the document loader, LLM provider,
and FalkorDB deployment have capacity. A higher value increases simultaneous
document pipelines and their memory use; it does not make one document's
pipeline run in parallel.

This is separate from extraction concurrency. `GraphExtraction` limits the
parallel LLM calls made for chunks *within* each document; the configured LLM
provider also has its own `max_concurrency` limit. Tune document-level and
LLM-level concurrency together, and lower either value if the provider starts
returning rate-limit errors.

```python
from graphrag_sdk.ingestion.extraction_strategies.graph_extraction import GraphExtraction

extractor = GraphExtraction(llm=llm, max_concurrency=6)
results = await rag.ingest(sources, extractor=extractor, max_concurrency=3)
```

For incremental refreshes, `apply_changes()` has the same
`max_concurrency` control for newly added documents. Keep
`update_concurrency` at its default of `1` unless you have verified that
concurrent updates are safe for your workload.
...
| `LLMVerifiedResolution` | Multi-document ingestion where names vary. Costs embedding calls, plus LLM calls for ambiguous pairs. Cross-document dedup is `finalize()`'s resolver and judge phases |
...
| [`resolution_strategies/llm_verified_resolution.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/ingestion/resolution_strategies/llm_verified_resolution.py) | LLMVerifiedResolution |
| [`storage/judge_dedup.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/storage/judge_dedup.py) | LLMJudgeDeduplicator — cross-document dedup at `finalize()` |
