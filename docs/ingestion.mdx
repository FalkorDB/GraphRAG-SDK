...
- The judge LLM partitions each set into same-referent groups; with `judge_vote=True` (default) a second pass over shuffled sets must agree, with `judge_vote=False` the single pass decides
