...
- Pairs at or above `hard_threshold` (0.95) merge outright; pairs down to `soft_threshold` (0.80) go to the LLM for a YES/NO verdict
...
- The judge LLM partitions each set into same-referent groups; a second pass over shuffled sets must agree
...
- `finalize(judge=False)` skips it; `judge_llm=` picks the model (gpt-4.1-class recommended); pass `resolver=LLMVerifiedResolution(...)` to `ingest()` to also resolve per document
