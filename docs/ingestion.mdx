...
**Default: LLMVerifiedResolution**
- Exact-match merge by `(normalized name, label)` first — no LLM or embedder needed
- Embeds surviving names within each label; pairs at or above `hard_threshold` (0.95) merge outright, pairs between `soft_threshold` (0.65) and 0.95 go to the LLM as batched YES/NO questions (5 pairs per call)
- A cross-label pass then asks the LLM about same-thing/different-type pairs (`Fresnel lens` as Product and Technology)
- Survivor keeps the merged descriptions joined with `" | "` and records absorbed labels in `merged_labels`
- Remaps all relationship endpoints to the survivor and deduplicates relationships by `(start_id, type, end_id)`

**Alternative: ExactMatchResolution**
- Groups by `(normalized name, label)` only — same name but different labels stay separate (e.g., Person "Paris" vs Location "Paris")
- No embeddings, no LLM calls; zero added cost
- Pass `resolver=ExactMatchResolution()` to `ingest()` when extraction is consistent or cost matters more than recall
...
|----------|-------------|
| `LLMVerifiedResolution` (default) | Embedding + LLM verification. Catches surface variants and descriptive references |
| `ExactMatchResolution` | Fast, deterministic, no LLM. Good when extraction is consistent or cost matters |
...
| [`resolution_strategies/llm_verified_resolution.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/ingestion/resolution_strategies/llm_verified_resolution.py) | LLMVerifiedResolution (default) |
