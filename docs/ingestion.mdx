---
title: "Ingestion"
description: "How documents become a knowledge graph through the 9-step ingestion pipeline."
---
...
- Runs after the exact-name dedup phase, over every entity of every document
...
- Both passes agree → **merge**: survivor = longest description, descriptions joined with `" | "`, every member's label added, other names kept in `aliases`, edges moved first, then the loser deleted
...
|----------|-------------|
| `ExactMatchResolution` (ingest default) | Fast, deterministic, no LLM |
| `LLMVerifiedResolution` (opt-in) | Embedding + LLM verification per document. Cross-document dedup is `finalize()`'s judge phase |
...
| [`resolution_strategies/llm_verified_resolution.py`](https://github.com/FalkorDB/GraphRAG-SDK/blob/main/graphrag_sdk/src/graphrag_sdk/ingestion/resolution_strategies/llm_verified_resolution.py) | LLMVerifiedResolution (opt-in) |
