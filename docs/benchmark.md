...
- **Graph statistics** -- Nodes, edges, mentions
...
async def main():
    ...
    if not args.query_only:
        ...
        print("Post-ingestion: finalizing (dedup + embeddings + indexes)...")
        finalize_results = await rag.finalize()
        print(f"  Deduplicated {finalize_results['entities_deduplicated']} entities")
        print(f"  Embedded {finalize_results['entities_embedded']} entities")
        print(f"  Embedded {finalize_results['relationships_embedded']} relationships")
...
| **Extraction** | `GraphExtraction` | GLiNER2 entity NER (step 1) + LLM verify & relationship extraction (step 2) |
...
| Strategy | Class | Key Parameters |
|----------|-------|---------------|
| **Retrieval** | `MultiPathRetrieval` (default) | RELATES edge vector search, 2-path entity discovery, 4-path chunk retrieval, cosine reranking |
...
await rag.finalize()  # dedup + entity embeddings + relationship embeddings + indexes
```
