...
class TestExtractionReportReachesCaller:
    ...
    async def test_run_exposes_report_in_result_metadata(
        ...
        # Chunk uids are content-derived (#309), so compare shape, not literals.
        failed, rel_failed = report.pop("failed_chunks"), report.pop("relation_failed_chunks")
        assert report == {
            "chunks_attempted": 3,
            "chunks_skipped": 4,
            "extraction_failed": False,
        }
        assert len(failed) == 1 and len(rel_failed) == 1 and failed != rel_failed
        assert all(uid.startswith("chunk-") for uid in failed + rel_failed)
    ...
    async def test_run_exposes_total_failure(self, ctx, mock_graph_store, mock_vector_store):
        ...
        assert len(set(report["failed_chunks"])) == 3
