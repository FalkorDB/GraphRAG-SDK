...
# ── Ontology.from_schema_org ───────────────────────────────────────


class TestFromSchemaOrg:
    """Curated Schema.org subset returned by ``Ontology.from_schema_org``."""

    def test_returns_non_empty_ontology(self) -> None:
        ont = Ontology.from_schema_org()
        assert len(ont.entities) >= 5
        assert len(ont.relations) >= 5

    def test_includes_core_entity_labels(self) -> None:
        ont = Ontology.from_schema_org()
        labels = {e.label for e in ont.entities}
        assert {"Person", "Organization", "Place", "Event", "CreativeWork"} <= labels

    def test_pattern_closure(self) -> None:
        """Every relation pattern must reference declared entity labels."""
        ont = Ontology.from_schema_org()
        declared = {e.label for e in ont.entities}
        for r in ont.relations:
            for src, tgt in r.patterns:
                assert src in declared, f"{r.label} src '{src}' not declared"
                assert tgt in declared, f"{r.label} tgt '{tgt}' not declared"

    def test_attribute_types_are_valid(self) -> None:
        """No attribute uses a type outside the SDK's allowed set."""
        from graphrag_sdk.core.models import _PROPERTY_TYPES

        ont = Ontology.from_schema_org()
        for e in ont.entities:
            for a in e.properties:
                assert a.type in _PROPERTY_TYPES, (
                    f"{e.label}.{a.name} has invalid type {a.type!r}"
                )

    def test_round_trips_through_file(self, tmp_path: Path) -> None:
        """Lossless save_to_file -> from_file round-trip."""
        ont = Ontology.from_schema_org()
        path = tmp_path / "schema_org.json"
        ont.save_to_file(str(path))
        reloaded = Ontology.from_file(str(path))
        assert reloaded.model_dump() == ont.model_dump()

    def test_merges_with_user_extensions(self) -> None:
        """The standard subset should compose cleanly with hand-added types
        via ``Ontology.merge`` — typical user pattern: start from
        Schema.org, add domain-specific entities on top."""
        base = Ontology.from_schema_org()
        extra = Ontology(
            entities=[
                Entity(
                    label="Patent",
                    properties=[Attribute(name="filing_date", type="DATE")],
                )
            ],
            relations=[
                Relation(
                    label="PATENTED_BY",
                    patterns=[("Patent", "Person")],
                )
            ],
        )
        merged = base.merge(extra)
        labels = {e.label for e in merged.entities}
        assert "Patent" in labels
        assert {"Person", "Organization", "Place"} <= labels
        rel_labels = {r.label for r in merged.relations}
        assert "PATENTED_BY" in rel_labels
        assert "WORKS_AT" in rel_labels  # preserved from base
