...
class TestPipelineParallelSteps:
    async def test_parallel_steps_same_results(self, pipeline, pipeline_components, ctx):
        """Parallel steps 8-11 should produce same results as sequential."""
        result = await pipeline.run("test.txt", ctx)

        # All 4 parallel steps should have executed
        pipeline_components["vector_store"].index_chunks.assert_called_once()
        pipeline_components["vector_store"].index_facts.assert_called_once()
        assert "facts_indexed" in result.metadata
        assert "synonym_edges_created" in result.metadata
    ...
    async def test_all_four_steps_called(self, pipeline, pipeline_components, ctx):
        """All 4 parallel steps (facts, synonymy, mentions, chunks) should be called."""
        result = await pipeline.run("test.txt", ctx)
