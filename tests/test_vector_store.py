...
class TestVectorStoreIndex:
    async def test_create_vector_index(self, vector_store, mock_connection):
        ...
        assert "vector.createNodeIndex" in cypher
...
class TestVectorStoreIndexChunks:
    async def test_index_chunks(self, vector_store, mock_connection, embedder):
        chunks = TextChunks(
            chunks=[
    ...
    async def test_index_chunks_error_continues(self, vector_store, mock_connection, embedder):
        ...
        result = await vector_store.index_chunks(chunks)
        assert result == 1  # first failed, second ok
